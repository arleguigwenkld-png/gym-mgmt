    ...
    if (empty($id_number) || empty($family_name) || empty($given_name) || empty($membership_availed) || $membership_rate <= 0 || empty($membership_status)) {
        ...
        if ($stmt_check->get_result()->num_rows > 0) {
            ...
            if ($stmt_update->execute()) {
                ...
                header("Location: expired_client.php?page=" . (isset($_GET['page']) ? $_GET['page'] : 1));
                exit();
            } else {
...
$sql_total = "SELECT COUNT(*) as total FROM clients WHERE membership_status = 'Expired'";
$total_result = $conn->query($sql_total);
$total_expired_clients = $total_result->fetch_assoc()['total'];
$total_pages = ceil($total_expired_clients / $records_per_page);

// Fetch expired clients
$sql = "SELECT * FROM clients WHERE membership_status = 'Expired' LIMIT ? OFFSET ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ii", $records_per_page, $offset);
$stmt->execute();
$expired_clients = $stmt->get_result();
$stmt->close();
...
<body class="bg-gradient-to-br from-blue-100 to-green-100 min-h-screen flex">
    ...
    <div class="flex-1 sm:ml-64">
                ...
                <form method="POST" action="">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" id="edit_activation_date" name="activation_date" class="mt-1 w-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
        ...
        <!-- Expired Clients Content -->
        <div class="container mx-auto max-w-7xl p-4 sm:p-8">
            <div class="bg-white p-8 rounded-xl shadow-lg">
                <h2 class="text-3xl font-bold mb-6 text-gray-800 text-center">Expired Clients</h2>
                <div class="overflow-x-auto">
                    <table class="min-w-full bg-white border border-gray-300 rounded-lg">
                        ...
                        <tbody>
                            <?php
                            if ($expired_clients->num_rows == 0) {
                                echo "<tr><td colspan='14' class='border px-4 py-3 text-gray-600 text-sm text-center'>No expired clients found.</td></tr>";
                            } else {
                                $row_number = $offset + 1;
                                while ($row = $expired_clients->fetch_assoc()) {
                                    ?>
